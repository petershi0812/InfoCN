...
public class GetComboxSelections extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		//System.out.println(request.getParameter("rights"));
		...
		if(word_id.equals("province")){			
			...
			sql = "SELECT * FROM DBO.GETADMINCITY('"+username+"','"+arrrightspk[0].trim()+"','"+arrrightspk[1].trim()+"','"+province_id+"')";
			variable = "city";
			tempResponse=node.getComboResult(sql,variable); 
		}
			...
			sql = "SELECT * FROM DBO.GETADMINDISTRICT('"+username+"','"+arrrightspk[0].trim()+"','"+arrrightspk[1].trim()+"','"+province_id+"','"+city_id+"')";
			variable = "district";			
			tempResponse=node.getComboResult(sql,variable); 
		}
