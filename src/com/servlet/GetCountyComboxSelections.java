...
public class GetCountyComboxSelections extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		if(word_id.equals("province")){			
			...
			sql = "SELECT * FROM DBO.GETADMINCITY_COUNTY('"+username+"','"+arrrightspk[0].trim()+"','"+arrrightspk[1].trim()+"','"+province_id+"')";
			variable = "city";
			tempResponse=node.getComboResult(sql,variable); 
		}
			...
			sql = "SELECT * FROM DBO.GETADMINCOUNTY('"+username+"','"+arrrightspk[0].trim()+"','"+arrrightspk[1].trim()+"','"+province_id+"','"+city_id+"')";
			variable = "district";			
			tempResponse=node.getComboResult(sql,variable); 
