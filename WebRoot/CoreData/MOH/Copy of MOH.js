Ext.onReady(function(){ 
   ...
   ****************************/				    
    ...
    var city_store = new Ext.data.SimpleStore({
    	fields:['city_id'],
    	proxy : new Ext.data.HttpProxy({ url : 'GetComboxSelections?id=city&rights='+rightspk}),
   		baseParams:{province_id:''},
 		remoteSort:true        
    }); 
    var district_store = new Ext.data.SimpleStore({
    	fields:['district_id'],
    	proxy : new Ext.data.HttpProxy({ url : 'GetComboxSelections?id=district&rights='+rightspk}),
   		baseParams:{province_id:'',city_id:''},
   		remoteSort:true      
    });	
    
	var hoplevel_store = new Ext.data.SimpleStore({
		fields: ['hos_level_id','hos_level'],
		data : [
				[0,'三级'],[1,'三级甲等'],[2,'三级其他'],[3,'二级'],[4,'一级'],[5,'未评']
		       ]
	});	
	var hoptype_store = new Ext.data.SimpleStore({
		fields: ['hos_type_id','hos_type'],
		data : [
				[3,"综合医院"],	[4,"中医医院"],	[5,"中西医结合医院"],	[6,"民族医院"],	[8,"口腔医院"],	[9,"眼科医院"],	[10,"耳鼻喉科医院"],	[11,"肿瘤医院"],	[12,"心血管病医院"],	[13,"胸科医院"],	[14,"血液病医院"],	[15,"妇产（科）医院"],	[16,"儿童医院"],	[17,"精神病医院"],	[18,"传染病医院"],	[19,"皮肤病医院"],	[20,"结核病医院"],	[21,"麻风病医院"],	[22,"职业病医院"],	[23,"骨科医院"],	[24,"康复医院"],	[25,"整形外科医院"],	[26,"美容医院"],	[27,"其他专科医院"],	[28,"护理院"]
		       ]
	});
	var hopproperty_store = new Ext.data.SimpleStore({
		fields: ['hos_property_id','hos_property'],
		data : [
				[1,'公立'],[2,'民营']
		       ]
	});	    
	...
	{ 
		var new_params={sql:grid_sql,dbname:database,numcolumnpoint:FieldStr_Point+12,rights:rightspk}; 
		Ext.apply(options.params,new_params); 
	});	
			
	var grid_sm = new Ext.grid.CheckboxSelectionModel();   		
    var grid_colModel= new Ext.grid.ColumnModel(fun_grid_colModel_config("MOH",grid_sm));
	    ...
	    tbar: [
	    {
	            ...
	            items: [
	                ...
	                listeners:{
		            	...
		            	change:function(){
		            		...
		            		if(province_field.isValid()){
		            			...
		            			district_field = Ext.getCmp('combo_district');
		            			district_field.clearValue();	            			
		            			var value = province_field.getValue(); 	            			
		            			//var index = province_store.find('province_id',value);
		            			if(value.length>=0){
	                ...
	                listeners:{
		            	...
		            	change:function(){
		            		...
		            		if(city_field.isValid()){
		            			...
		            			//var index = city_store.find('city_id',value);
		            			if(value.length>=0){
	            ...
	            },{text: '',disabled : true},{text: '',disabled : true},
	             {
	                ...
	                emptyText:'选择等级',              
	                allowBlank:true,
	                store:hoplevel_store,
	                mode: 'local',
	                triggerAction: 'all',
	                displayField: 'hos_level', 
	                valueField : 'hos_level_id',
	                forceSelection: false,
	                resizable:true,
	                style : 'margin-left : 3px',
	                listeners:{}
	            },                            
	            {
	                ...
	                mode: 'local',
	                triggerAction: 'all',
	                displayField: 'hos_type', 
	                valueField : 'hos_type_id',
	                forceSelection: false,
	                resizable:true,
	                style : 'margin-left : 6px'
	            },
	...
	var combinedchart_winpanel = new Ext.Window({ 
	   ...
	   ****************************/    
	   function fun_combox_search(){
   		...
   		var district_value = Ext.getCmp('combo_district').getRawValue();
   		var hoplevel_value = Ext.getCmp('hoplevel_combox').getRawValue();   		
   		var hoptype_value = Ext.getCmp('hoptype_combox').getRawValue();
   		var sql_str = '',sql_str_province = '',sql_str_city = '',sql_str_district = '',sql_str_grade = '',sql_str_type = '';
   		if(province_value == '' && hoplevel_value == '' && hoptype_value == ''){
   			Ext.Msg.alert('提示','请输入条件!');
   		}
   		else{
   			if(province_value.indexOf('(全国)')>=0 || city_value.indexOf('(全国)')>=0 || district_value.indexOf('(全国)')>=0){
   				sql_str_province = ' ';
   				sql_str_city = ' ';
   				sql_str_district= ' ';
   			} 
   			...
   			if(district_value != ''){
   				if(district_value.indexOf(",")>0){
					var strarray=district_value.split(",");
					for(var i = 0 ; i<= strarray.length-1 ; i++){
						if(i < strarray.length-1) sql_str_district = sql_str_district + "district = '" + strarray[i].replace(" ","") + "' or ";
						else sql_str_district = sql_str_district + "district = '" + strarray[i].replace(" ","") + "' ";
					}   				
   				}
   				else{
   					sql_str_district = "district = '"+district_value+"' ";	
   				} 
   				sql_str_district = " ( " + sql_str_district + " ) and ";
   				//alert(sql_str_district);  	
	   			} 
	   		}
   			if(hoplevel_value != ''){
   				if(hoplevel_value.indexOf(",")>0){
   					var strarray=hoplevel_value.split(",");
   					for(var i = 0 ; i<= strarray.length-1 ; i++){      											
   						switch (strarray[i].replace(" ",""))
					{
					case '三级':
					  sql_str_grade = sql_str_grade + "(Grade_Level_Link = '三级') or ";
					  break;					
					case '三级甲等':
					  sql_str_grade = sql_str_grade + "(Grade_Level_Link = '三级' and Grade_Link = '甲等') or ";
					  break;
					case '三级其他':
					  sql_str_grade = sql_str_grade + "(Grade_Level_Link = '三级' and Grade_Link = '乙、丙或未评等') or ";
					  break;
					case '二级':
					  sql_str_grade = sql_str_grade + "(Grade_Level_Link = '二级' and Grade_Link = '全部') or ";
					  break;
					case '一级':
					  sql_str_grade = sql_str_grade +  "(Grade_Level_Link = '一级' and Grade_Link = '全部') or ";
					  break;
					case '未评':
					  sql_str_grade = sql_str_grade +  "(Grade_Level_Link = '未评' and Grade_Link = '全部') or ";
						  break;						
						}
   					}
   				}else{
   					switch (hoplevel_value)
					{
					case '三级':
				  sql_str_grade = "Grade_Level_Link = '三级' ";
				  break;					
				case '三级甲等':
				  sql_str_grade = "Grade_Level_Link = '三级' and Grade_Link = '甲等' ";
				  break;
				case '三级其他':
				  sql_str_grade = "Grade_Level_Link = '三级' and Grade_Link = '乙、丙或未评等' ";
				  break;
				case '二级':
				  sql_str_grade = "Grade_Level_Link = '二级' and Grade_Link = '全部' ";
				  break;
				case '一级':
				  sql_str_grade = "Grade_Level_Link = '一级' and Grade_Link = '全部' ";
				  break;
				case '未评':
				  sql_str_grade = "Grade_Level_Link = '未评' and Grade_Link = '全部'";
				  break;
				}
				sql_str_grade = sql_str_grade + " or ";
   				}
   				sql_str_grade = " ( " + sql_str_grade.substring(0,sql_str_grade.length-3) + " ) and ";
   			}
   			if(hoptype_value != ''){
   				if(hoptype_value.indexOf(",")>0){
				var strarray=hoptype_value.split(",");
				for(var i = 0 ; i<= strarray.length-1 ; i++){
					if(i < strarray.length-1) sql_str_type = sql_str_type + "Hop_Type = '" + strarray[i].replace(" ","") + "' or ";
					else sql_str_type = sql_str_type + "Hop_Type = '" + strarray[i].replace(" ","") + "' ";
					}   				
   				}
   				else{
   					sql_str_type = "Hop_Type = '"+hoptype_value+"' ";	
   				} 
   				sql_str_type = " ( " + sql_str_type + " ) and ";		
   			}   	
   			sql_str = sql_str_province+sql_str_city+sql_str_district+sql_str_grade+sql_str_type;
   			sql_str = sql_str.substring(0,sql_str.length-4) /*+ ' order by Total_Income desc'*/;
	...
	function fun_clear_filter(){
	    ...
	    function callback(id){
	   		if(id == 'yes'){	   
		        ...
		        Ext.getCmp('combo_district').reset();   
		        Ext.getCmp('hoptype_combox').reset();
		        Ext.getCmp('hoplevel_combox').reset();  		        
				grid_sql = ' '; 
				quick_filter_sql = ' '; 
		    	filter_module_sql = ' ';     
		    	grid_store_reload(grid_store,pageSize,PagingToolbar,database,grid_sql + '@' + quick_filter_sql + '@' + filter_module_sql + '@' + sort_module_sql + '@' + newcolumn_module_sql,rightspk);  	
