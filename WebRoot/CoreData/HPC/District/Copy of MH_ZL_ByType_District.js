Ext.onReady(function(){ 
   ...
   ****************************/
	...
	var database = 'hpcmzlbytypedistrict';
	var SelectionPoint = 0;
	var FieldStr_Point = 7;	
   ...
   ****************************/				    
	...
	var hoptype_store = new Ext.data.SimpleStore({
		...
		data : [
				[2,"医院总计"],[3,"综合医院"],	[4,"中医医院"],	[5,"中西医结合医院"],	[6,"民族医院"],	[7,"专科医院小计"],[8,"口腔医院"],	[9,"眼科医院"],	[10,"耳鼻喉科医院"],	[11,"肿瘤医院"],	[12,"心血管病医院"],	[13,"胸科医院"],	[14,"血液病医院"],	[15,"妇产（科）医院"],	[16,"儿童医院"],	[17,"精神病医院"],	[18,"传染病医院"],	[19,"皮肤病医院"],	[20,"结核病医院"],	[21,"麻风病医院"],	[22,"职业病医院"],	[23,"骨科医院"],	[24,"康复医院"],	[25,"整形外科医院"],	[26,"美容医院"],	[27,"其他专科医院"],	[28,"护理院"]
		       ]
	});	    
    ...
    var grid_colModel= new Ext.grid.ColumnModel(fun_grid_colModel_config("HPC_MH_ZL_ByType_District",grid_sm));
	...
	var combinedchart_winpanel = new Ext.Window({ 
	   ...
	   ****************************/    
	   function fun_combox_search(){
	   		...
	   		var hoptype_value = Ext.getCmp('hoptype_combox').getValue();
	   		var sql_str = '',sql_str_province = '',sql_str_city = '',sql_str_district = '',sql_str_type = '';
	   		if(province_value == ''  && hoptype_value == ''){
	   			...
	   			if(hoptype_value != ''){
	   				if(hoptype_value.indexOf(",")>0){
						...
						for(var i = 0 ; i<= strarray.length-1 ; i++){
							if(i < strarray.length-1) sql_str_type = sql_str_type + "Hop_Type_Link = '" + strarray[i].replace(" ","") + "' or ";
							else sql_str_type = sql_str_type + "Hop_Type_Link = '" + strarray[i].replace(" ","") + "' ";
						}   				
	   				}
	   				else{
	   					sql_str_type = "Hop_Type_Link = '"+hoptype_value+"' ";	
	   				} 
	   				sql_str_type = " ( " + sql_str_type + " ) and ";		
		   			}  	   			
		   			sql_str = sql_str_province+sql_str_city+sql_str_district + sql_str_type;
