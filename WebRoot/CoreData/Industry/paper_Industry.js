Ext.onReady(function(){ 
   ...
   ****************************/
	...
	var database = 'paperindustrydb';  //industrydb
	var SelectionPoint = 2;
	var FieldStr_Point = 9;	
   ...
   ****************************/
    ...
    var grid_colModel= new Ext.grid.ColumnModel(fun_grid_colModel_config("paper_Industry_dataset",expander));
	...
	var grid = new Ext.grid.GridPanel({	
	    ...
	    tbar: [	   	    
            ...
            },
			{
                text: '取消筛选',
                scale: 'large',
                tooltip:'取消快速筛选和筛选的所有条件',
                rowspan: 2,
                iconCls: 'large_clear_filter',
                iconAlign: 'left',
                handler : fun_clear_filter
            }]
        },                
        {
            xtype: 'buttongroup',
...
/*		view: new Ext.ux.grid.BufferView({  	         
   ...
   ***************************/	 	
	...
	function Create_pdfWindow(papertitle){
		if (Ext.getCmp("pdf_winpanel") != undefined) {
		    Ext.getCmp("pdf_winpanel").destroy();
		}					
		var pdf_winpanel = new Ext.Window({  
		        id:'pdf_winpanel',
		        width : 850,  
		        height: 500, 
		        resizable:true,
				...
				fill : true,
	      		layout : 'fit',
	      		items:[{html:'<iframe src=Industry/paper/Report.jsp?paperid=' +papertitle + ' width="100%" height="100%" frameborder="no" scrolling="yes"></iframe>'}]
		});		
	}
	function fun_large_pdf(){
		var selectionmodel = grid.getSelectionModel();
		var selectioncount = selectionmodel.getCount();	
		if(selectioncount > 0){
			var record = selectionmodel.getSelected();
			papertitle = record.get("PaperDownloadDummyname");
			Create_pdfWindow(papertitle);	
			Ext.getCmp('pdf_winpanel').show();				
		}
		else{
			Ext.Msg.alert('提示','请选择需要浏览的报告!');
		}			
	}			
	/*******************************
	 * ## 记录登录时间   
