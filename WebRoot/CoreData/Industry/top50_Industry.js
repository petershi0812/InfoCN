Ext.onReady(function(){ 
   ...
   ****************************/
	...
	var database = 'top10industrydb|30';  //industrydb
	var SelectionPoint = 0;
	var FieldStr_Point = 33;	
   ...
   ****************************/				    
    ...
    var grid_colModel= new Ext.grid.ColumnModel(fun_grid_colModel_config("top10_Industry_dataset",grid_sm));
	...
	var grid = new Ext.grid.GridPanel({	
        ...
        listeners:{
			...
			'rowdblclick' : function(grid,rowIndex,e) {
				...
				zjzj = grid_store.getAt(rowIndex).get('zjzj');
				gyzcz = grid_store.getAt(rowIndex).get('gyzcz');
				cyrs = grid_store.getAt(rowIndex).get('cyrs');					
				dist1['key_indicators'] = [yysr,yylr,zjzj,gyzcz,cyrs];
				//主营业务板块
				zyywhd1 = grid_store.getAt(rowIndex).get('zyywhd1');
				zyywhd2 = grid_store.getAt(rowIndex).get('zyywhd2');
				zyywhd3 = grid_store.getAt(rowIndex).get('zyywhd3');
				dist1['major_activity'] = [zyywhd1,zyywhd2,zyywhd3];
				//主营构成
				dist1['peichart'] = [yylr,yysr];
				//行业地位
				ylze = grid_store.getAt(rowIndex).get('ylze');
				yycb = grid_store.getAt(rowIndex).get('yycb');
				syzqyhj = grid_store.getAt(rowIndex).get('syzqyhj');				 
				zjzj = grid_store.getAt(rowIndex).get('zjzj');				
				ldzchj = grid_store.getAt(rowIndex).get('ldzchj');
				dist1['radarchart'] = [cyrs,gyzcz,ylze,yylr,yycb,yysr,syzqyhj,zjzj,ldzchj];											
				companyname = grid_store.getAt(rowIndex).get('dwmc1');
				industrycode = grid_store.getAt(rowIndex).get('IndustryCode');				
				yearid = grid_store.getAt(rowIndex).get('Year_id');	
				
	    		Ext.Ajax.request({      
	    			 url: 'GetSinglCompanyInfo',
		             params: {company:companyname,industrycode:industrycode},      
				...
				function gg(returndata){					
	        		var companyarr = returndata.split("^");
	             	dist1['companybrief'] = companyarr[0];
	             	dist1['companymajorinfo'] = companyarr[1];
	             	dist1['companyequity'] = companyarr[2];
	             	dist1['companyevent'] = companyarr[3];
	             	dist1['industryavgindicators'] = companyarr[4];
	             	
					//new tab
					Ext.get('content-iframe').dom.src = tabid; 	
					var n = centerPanel.getComponent(tabid); 
			        if (!n&&tabid) {
			        	var dynamicPanel = renderCompanyInfo(tabid,dist1);		        	
		    		   ...
		    		   /***************************
					     * ## 主营业务饼图 
					   ****************************/	
					    rendepieECharts("piechartContainer" + tabid,dist1); 
					   /***************************
					     * ## 行业地位雷达图 
					   ****************************/		    
					    renderradarECharts("radarchartContainer" + tabid,dist1);       
		        	return true; 	        	
				}	        		        		       
			}
        }
...
'-',
   ...
   ****************************/
    var centerPanel = new Ext.TabPanel({
	    ...
	    items:[{
	            title:'<center>国家统计局规模以上企业</center>',
	            id:'content-iframe', 	            
	            iconCls : 'company',
	            layout : 'fit',
	            tabTip   : '企业列表',
