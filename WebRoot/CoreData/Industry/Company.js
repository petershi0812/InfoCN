Ext.onReady(function(){ 
   ...
   ****************************/
	...
	var database = 'ppcompanyInfodb';  //industrydb
	var SelectionPoint = 0;
	var FieldStr_Point = 31;	
   ...
   ****************************/				    
	...
	grid_store.setDefaultSort('yysr','DESC');
			
	var grid_sm = new Ext.grid.CheckboxSelectionModel();
    var grid_colModel= new Ext.grid.ColumnModel(fun_grid_colModel_config("ppcompany_dataset",grid_sm));
	...
	var grid = new Ext.grid.GridPanel({	
	    ...
	    tbar: [	   	    
           ...
           /* {
            ...
            },*/
			{
                ...
                iconCls: 'showcolumn',
                menu: [
                	{text: '显示所有指标', tooltip:'显示所有的指标列',iconCls: 'small_showcolumn',handler : fun_show_allcolumn}
                	//{text: '显示主要指标', tooltip:'显示主要的指标列',iconCls: 'small_showpartcolumn',handler : fun_show_primarycolumn}
                ]
            } ,          
            {
        ...
        listeners:{
			...
			'rowdblclick' : function(grid,rowIndex,e) {				
				...
				tabid = grid_store.getAt(rowIndex).get('ID');
				tabpanelid = "tabpanel" + tabid;
				yearid = grid_store.getAt(rowIndex).get('Year_id');
				//公司简介
				companydictionary['companybrief'] = grid_store.getAt(rowIndex).get('gsjj');
				//公司名称
				companyname = grid_store.getAt(rowIndex).get('dwmc1');
				//关键指标
				yysr = grid_store.getAt(rowIndex).get('yysr');
				yycb = grid_store.getAt(rowIndex).get('yycb');
				yylr = grid_store.getAt(rowIndex).get('yylr');				
				xsmll = grid_store.getAt(rowIndex).get('xsmll');
				ebit = grid_store.getAt(rowIndex).get('ebit');
				ebitda = grid_store.getAt(rowIndex).get('ebitda');
				ebitmargin = grid_store.getAt(rowIndex).get('ebitmargin');
				roic = grid_store.getAt(rowIndex).get('roic');
				ros = grid_store.getAt(rowIndex).get('ros');	
				yyzcb = grid_store.getAt(rowIndex).get('yyzcb');
				
				companydictionary['key_indicators'] = [yysr,yycb,yylr,xsmll,ebit,ebitda,ebitmargin,roic,ros,yyzcb];
				//公司主要信息
				gszch = grid_store.getAt(rowIndex).get('gszch');
				zzjgdm = grid_store.getAt(rowIndex).get('zzjgdm');
				ispp = grid_store.getAt(rowIndex).get('ispp');
				dh = grid_store.getAt(rowIndex).get('dh');
				gzdz = grid_store.getAt(rowIndex).get('gzdz');
				zcsj = grid_store.getAt(rowIndex).get('zcsj');
				frdb = grid_store.getAt(rowIndex).get('frdb');
				qylx = grid_store.getAt(rowIndex).get('qylx');				
				companydictionary['companymajorinfo'] = [gszch,zzjgdm,ispp,dh,gzdz,zcsj,frdb,qylx];				
				//公司股权结构
				zczb = grid_store.getAt(rowIndex).get('zczb');
				gd1jczbl = grid_store.getAt(rowIndex).get('gd1jczbl');
				gd2jczbl = grid_store.getAt(rowIndex).get('gd2jczbl');
				gd3jczbl = grid_store.getAt(rowIndex).get('gd3jczbl');
				gd4jczbl = grid_store.getAt(rowIndex).get('gd4jczbl');
				gd5jczbl = grid_store.getAt(rowIndex).get('gd5jczbl');
				gd6jczbl = grid_store.getAt(rowIndex).get('gd6jczbl');
				gd7jczbl = grid_store.getAt(rowIndex).get('gd7jczbl');
				gd8jczbl = grid_store.getAt(rowIndex).get('gd8jczbl');
				gd9jczbl = grid_store.getAt(rowIndex).get('gd9jczbl');
				gd10jczbl = grid_store.getAt(rowIndex).get('gd10jczbl');
				gd11jczbl = grid_store.getAt(rowIndex).get('gd11jczbl');
				gd12jczbl = grid_store.getAt(rowIndex).get('gd12jczbl');
				gd13jczbl = grid_store.getAt(rowIndex).get('gd13jczbl');
				gd14jczbl = grid_store.getAt(rowIndex).get('gd14jczbl');
				gd15jczbl = grid_store.getAt(rowIndex).get('gd15jczbl');
				companydictionary['companystockshare'] = [zczb,gd1jczbl,gd2jczbl,gd3jczbl,gd4jczbl,gd5jczbl,gd6jczbl,gd7jczbl,gd8jczbl,gd9jczbl,gd10jczbl,gd11jczbl,gd12jczbl,gd13jczbl,gd14jczbl,gd15jczbl];
				//private company finance indicators
				ldzchj = grid_store.getAt(rowIndex).get('ldzchj');
				ch = grid_store.getAt(rowIndex).get('ch');
				ldfzhj = grid_store.getAt(rowIndex).get('ldfzhj');
				zchj = grid_store.getAt(rowIndex).get('zchj');
				fzhj = grid_store.getAt(rowIndex).get('fzhj');
				yszk = grid_store.getAt(rowIndex).get('yszk');
				yfzk = grid_store.getAt(rowIndex).get('yfzk');
				gdzc = grid_store.getAt(rowIndex).get('gdzc');
				jlr = grid_store.getAt(rowIndex).get('jlr');
				syzqy = grid_store.getAt(rowIndex).get('syzqy');
				xsfy = grid_store.getAt(rowIndex).get('xsfy');
				glfy = grid_store.getAt(rowIndex).get('glfy');
				cwfy = grid_store.getAt(rowIndex).get('cwfy');
				//偿债能力分析
				/*偿债能力分析-流动比率*/
				ldratio = ldfzhj == null?0:(ldzchj / ldfzhj * 100).toFixed(2);				
				/*偿债能力分析-速动比率*/
				sdratio = ldfzhj == null?0:((ldzchj-ch) / ldfzhj * 100).toFixed(2);
				/*偿债能力分析-资产负债率*/
				zcfzratio = zchj == null?0:(fzhj / zchj * 100).toFixed(2);
				
				//运营能力分析
				/*运营能力分析-应收账款周转率*/
				yszktuneroverratio = yszk == null?0:(yysr/ yszk * 100).toFixed(2);			
				/*运营能力分析-应付账款周转率*/
				yfzktuneroverratio = yfzk == null?0:(yycb/ yfzk * 100).toFixed(2);
				/*运营能力分析-存货周转率*/
				chtuneroverratio = ch == null?0:(yycb/ ch * 100).toFixed(2);
				/*运营能力分析-流动资产周转率*/
				ldzctuneroverratio = ldzchj == null?0:(yysr/ ldzchj * 100).toFixed(2);
				/*运营能力分析-固定资产周转率*/
				gdzctuneroverratio = gdzc == null?0:(yysr/ gdzc * 100).toFixed(2);
				/*运营能力分析-总资产周转率*/
				zzctuneroverratio = zchj == null?0:(yysr/ zchj * 100).toFixed(2);	
				
				//盈利能力分析
				/*盈利能力分析-净资产收益率*/
				jzcroi = syzqy == null?0:(jlr / syzqy * 100).toFixed(2);
				/*盈利能力分析-营业利润率*/
				yyrevenueratio = yysr == null?0:(yylr / yysr * 100).toFixed(2);
				/*盈利能力分析-销售费用率*/
				xsexpenseratio = yysr == null?0:(xsfy / yysr * 100).toFixed(2);
				/*盈利能力分析-管理费用率*/
				glexpenseratio = yysr == null?0:(glfy / yysr * 100).toFixed(2);
				/*盈利能力分析-财务费用率*/
				cwexpenseratio = yysr == null?0:(cwfy / yysr * 100).toFixed(2);
				/*盈利能力分析-三项费用率*/
				totalexpenseratio = yysr == null?0:((xsfy + glfy + cwfy) / yysr / 10000 * 100).toFixed(2);
								
				companydictionary['companyfinanceindicators'] = 
					[
						['偿债能力分析',['流动比率','速动比率','资产负债率'],[ldratio,sdratio,zcfzratio]],
						['运营能力分析',['应收账款','应付账款','存货','流动资产','固定资产','总资产'],[yszktuneroverratio,yfzktuneroverratio,chtuneroverratio,ldzctuneroverratio,gdzctuneroverratio,zzctuneroverratio]],
						['盈利能力分析',['三项费用率','财务费用率','管理费用率','销售费用率','营业利润率','净资产收益率'],[totalexpenseratio,cwexpenseratio,glexpenseratio,xsexpenseratio,yyrevenueratio,jzcroi]]						
					];			
								
	    		Ext.Ajax.request({      
	    			 url: 'GetSinglePPCompanyInfo',
		             params: {year:yearid,company:companyname},      
		             callback : function(options, success, response) {
		             	var returndata = trim(response.responseText);
						companydictionary['companybusinessinfo'] = returndata.split("^")[0];
						companydictionary['companyevent'] = returndata.split("^")[1];
						companydictionary['companypiechart'] = returndata.split("^")[2];
						companydictionary['companypatents'] = returndata.split("^")[3];	
		             	gg(companydictionary);		             	
		             }
		        });
		        
				function gg(companydictionary){						             
					//new tab
					Ext.get('content-iframe').dom.src = tabid; 	
					var n = centerPanel.getComponent(tabid); 
			        if (!n&&tabid) {
			        	pp = cwfy = grid_store.getAt(rowIndex).get('ispp')=='上市' ? 'public' :'private';
			        	var dynamicPanel = renderPPCompanyInfo(tabid,companydictionary,pp);    			        	
			     		var n = centerPanel.add({   
			    			id:tabid,   
			    			layout:'fit',
			    			border:false,
			    			title:'<center>'+companyname+'</center>',  
			    			closeAction : 'close',
			    			closable:true,  
			    			tabTip : companyname,
			    			items:[dynamicPanel],
				    ...
				    rendePiePPECharts("yysrpieContainer" + tabid,companydictionary,0);
				    //rendePiePPECharts("srblpieContainer" + tabid,companydictionary,1);
				    //rendePiePPECharts("yycbpieContainer" + tabid,companydictionary,2);
				    //rendePiePPECharts("yylrpieContainer" + tabid,companydictionary,3);
				    //rendePiePPECharts("mlrpieContainer" + tabid,companydictionary,4);
		        	return true; 	        	
				}							
			}			
        }
   ...
   ****************************/
	...
	var large_filter_panel_gridpanel_cm=new Ext.grid.ColumnModel({
	    columns:[	       
			...
			{header:'<center>输入内容</center>',dataIndex:'contents',width : 100,align :'right',sortable : false,hideable: false,
				...
				renderer: function(value,metadata,record){
					if(!isNaN(value)) return Ext.util.Format.number(value, '0,000');
					else return value;
				}
			},
	...
	var combinedchart_winpanel = new Ext.Window({ 
	   ...
	   /***************************
	     ...
	     *   @@fun_show_primarycolumn: 主界面显示主要指标按钮
	...
	function fun_sort_submit(){	
		delete grid_store.sortInfo; //重要删除默认排序功能，否则不能自定义排序了
		large_sort_panel_store.commitChanges();
		var store_count = large_sort_panel_store.getCount();
	...
	function fun_show_primarycolumn(){
		var grid_col_model = Ext.getCmp("grid").getColumnModel();
	    var column_count = grid_col_model.getColumnCount();
	    for(var k = 17 ;k<=column_count-1;k++)
	    {
	...
	function fun_summary_submit(){
		...
		combox_displayfield = replaceAll(combox_displayfield,"\\[个\\]","");
		combox_displayfield = replaceAll(combox_displayfield,"\\[千元\\]","");
		combox_displayfield = replaceAll(combox_displayfield,"\\[百分之一\\]","");
		vcombox_displayfield = replaceAll(combox_displayfield,"\\[次\\]","");
