Ext.onReady(function(){
    ...
    Ext.BLANK_IMAGE_kk ='../EXT/extjs3.3.1/images/s.gif';  
    /***************************
     * @@rightspk: 权限值
    ***************************/
   ...
   /***************************
     ...
     *  @@ copyRowNum: 复制行数
     *  @@ grid_sql ： 主页的sql语句
     *  @@ tree_sql : tree对应node的sql语句,default:总计
     *  @@ tree_node : tree对应node名称
     ...
     *  @@ AllFieldStr_Point: 所有指标中使用数字格式dataindex的起始点
     *  @@ MOH_IsSame: grid行点击事件中判断是否是同一个省份@城市
     *  @@ activeaccording_id: 图表中的panel的id
   ...
   ****************************/
	...
	var tree_sql = '1';
	var tree_node = ' ';
	var quick_filter_sql = ' ';
    var sort_module_sql = ' ';
    var filter_module_sql = ' '; 
    var newcolumn_module_sql = ' ';
	var database = 'disease';
	var SelectionPoint = 0;
	var FieldStr_Point = 6;
	var AllFieldStr_Point = 8;
	var MOH_IsSame = '';
	...
	var ctype;
   /***************************
     * ## 主界面gridpanel中所有的store
   ****************************/
    var province_store = new Ext.data.SimpleStore({
    	fields:['province_id'],
        proxy : new Ext.data.HttpProxy({ url : 'GetComboxSelections?id=province'}),
        baseParams:{rights:rightspk}        
    });  
        
    var city_store = new Ext.data.SimpleStore({
    	fields:['city_id'],
    	proxy : new Ext.data.HttpProxy({ url : 'GetComboxSelections?id=city'}),
   		baseParams:{province_id:'',rights:rightspk},
 		remoteSort:true        
    }); 
   ...
   /***************************
     * ## 疾病数据库树形结构
   ****************************/    
   var West_DiseaseNativate_Loader= new Ext.tree.TreeLoader({  
        dataUrl : 'GetDiseaseSysFunctiontree?flag=0'
        });
	var West_DiseaseNativate_Root=new Ext.tree.AsyncTreeNode({
		expanded:true,
		loader:West_DiseaseNativate_Loader
		});
    var West_DiseaseNativate_Tree=new Ext.tree.TreePanel({        
        renderTo:"West_Nativate_Tree_Div",//如果使用renderTo，则不能使用setRootNode()方法，需要在TreePanel中设置root属性        
        plain:true,
        id : 'West_DiseaseNativate_Tree',
        baseCls:"x-plain",
        loader:West_DiseaseNativate_Loader,
        frame:true,
        autoHeight:true,
        ...
        border:false,
        rootVisible:false,
        root:West_DiseaseNativate_Root
    });
    West_DiseaseNativate_Tree.render();

	Ext.getCmp('West_DiseaseNativate_Tree').on('click',function(node){
		if(node.isLeaf()){
			rightspk = "10|"+node.id;
			province_store.reload({params : {rights:rightspk}});
		}
		else{
			rightspk = document.getElementById("rightspk").value; 
			province_store.reload({params : {rights:rightspk}});
		}
		tree_sql = node.attributes.id;		
		tree_node = node.attributes.text;			
		grid_store_reload(grid_store,pageSize,PagingToolbar,database,grid_sql + '@' + quick_filter_sql + '@' + filter_module_sql + '@' + sort_module_sql + '@' + newcolumn_module_sql + '@' + tree_sql,rightspk);
		/*疾病地图*/
		if (Ext.getCmp("morbidity_winpanel") != undefined) {
		    if(!Ext.getCmp('morbidity_winpanel').hidden){
				fun_morbidity();
			}
		}
		/*MOH remove*/
		if (Ext.getCmp("mohonlyread_grid_store") != undefined) {
		    Ext.getCmp("mohonlyread_grid_store").remove();
		}
		if (Ext.getCmp("mohonlyread_grid") != undefined) {
		    Ext.getCmp("mohonlyread_grid").destroy();
		}
		if (Ext.getCmp("hoprightMenu") != undefined) {
		    Ext.getCmp("hoprightMenu").destroy();
		}		
	});
   ...
   ****************************/   	
	var grid_store = new Ext.data.Store({//配置分组数据集		
		autoLoad: {params:{start:0,limit:pageSize,rights:rightspk}},
		reader: new Ext.data.JsonReader(),
		proxy : new Ext.data.HttpProxy({
			url : 'GetAllDiseaseGridstorePage'
		})
	});			
	grid_store.on('beforeload', function(store,options) 
	{ 
		var new_params={sql:grid_sql + '@' + quick_filter_sql + '@' + filter_module_sql + '@' + sort_module_sql + '@' + newcolumn_module_sql  + '@' + tree_sql,dbname:database,numcolumnpoint:AllFieldStr_Point}; 
		Ext.apply(options.params,new_params); 
	});	
	var grid_sm = new Ext.grid.CheckboxSelectionModel({singleSelect:false});   	
	var grid_colModel= new Ext.grid.ColumnModel(fun_grid_colModel_config("疾病",grid_sm));
	...
	var grid = new Ext.grid.GridPanel({	
	    ...
	    tbar: [{
            ...
            items: [
			{
                ...
                listeners:{
	            	...
	            	change:function(){
	            		province_field = Ext.getCmp('combo_province');
	            		if(province_field.isValid()){
	            			city_field = Ext.getCmp('combo_city');
	            			city_field.clearValue();	            			
	            			var value = province_field.getValue(); 	            			
	            			if(value.length>=0){
	            				Ext.getCmp('combo_province').setValue(value);
	            				city_store.baseParams.province_id=value;
	            				city_store.baseParams.rights=rightspk;
	            				city_store.load(); 
	            			}
	            		}
	            	} 
                ...
                listeners:{
	            	...
	            	change:function(){
	            		...
	            		if(city_field.isValid()){
	            			...
	            			if(value.length>=0){
	            				Ext.getCmp('combo_city').setValue(value);	            			
	            			}
	            		}
	            	}
                }                
            }, {text: '',disabled : true},{text: '',disabled : true} ,	            	         
            {
                text: '搜索',
                width : 30,
                id : 'combox_search',
            ...
            {
            	text: '初始页面', iconCls: 'initgrid',handler : fun_init_grid
            },
            {
              	text: '取消筛选', iconCls: 'clear_filter',handler : fun_clear_filter
            }]
        },                
        {
            xtype: 'buttongroup',
	 ...
	 grid.render();			
     grid.addListener('rowdblclick', fun_grid_rowclick);
  				
   /***************************
     * ## 首页整体布局
     * 	@@： West是疾病树形结构
     * 	@@： Center是疾病数据库以及对于医院数据库
   ****************************/			
      	...
      	items : [
            ...
            layoutConfig:{animate:true,fill:true},
      	  	...
      	  	items: [
      	  	  {
                border:false,
                items:[West_DiseaseNativate_Tree],
                containerScroll: true,
      	  		autoScroll:true, 
                iconCls:'plugin'   
              }              
   ...
   ****************************/
	var editorfield_combox_store = new Ext.data.SimpleStore({
	   ...
	   autoLoad : true,
	   data : [['行政区划代码','AdminCode'],['省份','Province'],['城市','City']]
	   });	
	...
	large_sort_panel_bar.add(       
	   {xtype : 'tbspacer'},'-',
	   {text : '加入条件',tooltip : '增加条件',iconCls : 'add',handler : fun_sort_add},
	   '-',
	   {xtype : 'tbspacer'},
	   {text : '删除条件',tooltip : '删除条件',iconCls : 'delete',handler : fun_sort_del},
	   '-',{xtype : 'tbspacer'},
	   {iconCls : 'arrow_up',text : '上移',tooltip : '上移',handler : fun_sort_moveup},
	   {xtype : 'tbspacer'},
	   {iconCls : 'arrow_down',text : '下移',tooltip :'下移',handler : fun_sort_movedown},
	   '-',{xtype : 'tbspacer'},'->',
	   {text : '确定',iconCls : 'accept',tooltip:'确定',handler : fun_sort_submit},
	   {xtype : 'tbspacer'},
	   {text : '取消',iconCls : 'deldt',tooltip : '清除所有条件',handler : fun_sort_cancel},
	   {xtype : 'tbspacer'}
	);    
	...
	//var onlyread_contents_textfield = new Ext.form.TextField({
	//   	allowBlank: false,       
	//   	id:'onlyread_contents_textfield'
	// });		
	...
	var large_filter_panel_onlyread_gridpanel_cm=new Ext.grid.ColumnModel({
	    columns:[	       
			...
			{header:'<center>设置的条件</center>',dataIndex:'CH_conditions',width : 100,align :'left',sortable : false,hideable: false/*,editor : onlyread_contents_textfield*/} ,
	...
	var large_filter_panel = new Ext.Panel({  
		...
		},
		{
			...
			fill : true,
	  		layout : 'fit',
			tbar : [
				{xtype : 'button',text : '删除',iconCls : 'sel_left',id : 'large_filter_panel_removesingle',handler : fun_filter_remove},
				'-',
				{xtype : 'button',text : '清空',iconCls :'select_clear',id : 'large_filter_panel_clear',handler : fun_filter_clear},
				'->','-',
				{xtype : 'button',text : '确定',iconCls :'accept',id : 'large_filter_panel_submit',handler : fun_filter_submit}
				...
				items : [
				{
					...
					height :160, 
		      		...
		      		items:[{
		                ...
		                bbar : [
		                	'->','-',
			                {xtype : 'button',text : '新增',iconCls : 'add',handler : fun_newcolumn_add},
			                {xtype : 'button',text : '删除',iconCls : 'delete',handler : fun_newcolumn_delete},
			                /*{xtype : 'button',text : '修改',iconCls : 'edit'},*/
			                '-',
			                //{xtype : 'button',text : '确定',iconCls : 'accept',handler : fun_newcolumn_submit}, 
			                {xtype : 'button',text : '关闭',iconCls : 'deldt',handler : fun_newcolumn_close}
		                ],
	...
	var combinedchart_winpanel = new Ext.Window({ 
	   ...
	   /***************************
	     ...
	     *   @@fun_grid_rowclick: grid行单击事件
	     *   	$$Create_GridPanel_Remote(_yearid__sql): 功能函数，动态创建gridpanel
	     *   		&&fun_moh_export: MOH右键导出按钮
	     * ## 该页面所有图表函数
	   ...
	   ****************************/
	   function fun_combox_search(){
	   		...
	   		var city_value = Ext.getCmp('combo_city').getRawValue();   		
	   		var sql_str = '',sql_str_province = '',sql_str_city = '';
	   		if(province_value == ''){
	   			Ext.Msg.alert('提示','请输入条件!');
	   		}
	   		else{
	   			if(province_value.indexOf('(全国)')>=0 || city_value.indexOf('(全国)')>=0){
	   				sql_str = ' ';
	   			}
	   			else{
		   			if(province_value != ''){
	   			...
	   			if(city_value != ''){
	   				if(city_value.indexOf(",")>0){
						var strarray=city_value.split(",");
						for(var i = 0 ; i<= strarray.length-1 ; i++){
							if(i < strarray.length-1) sql_str_city = sql_str_city + "city = '" + strarray[i].replace(" ","") + "' or ";
							else sql_str_city = sql_str_city + "city = '" + strarray[i].replace(" ","") + "' ";
						}   				
	   				}
	   				else{
	   					sql_str_city = "city = '"+city_value+"' ";	
	   				} 
	   				sql_str_city = " ( " + sql_str_city + " ) and ";
	   				//alert(sql_str_city);
		   			}
		   			sql_str = sql_str_province+sql_str_city;
		...
		if(store_count != 0){
			...
			for(var i = 0 ;i<=store_count -1 ; i++){
				...
				if(!fun_gridfieldname_exists(trim(fieldname),grid)){
					...
					var new_field = "{header:'"+fieldname+"',align:'right',width: 100,xtype:'numbercolumn',format : '0,00.000',sortable:true,hideable:false,dataIndex:'"+fieldname+"'}";
	...
	function fun_grid_rowclick(grid, rowindex, e){		
		var select_model = grid.getSelectionModel();
		var record = select_model.getSelected();
		if(record){
			var province_city = record.get('Province') + '@' + record.get('City');
			var year_id = record.get('Year_id');
			if(province_city != MOH_IsSame){
				MOH_IsSame = province_city;
				if (Ext.getCmp("mohonlyread_grid_store") != undefined) {
					mohonlyread_grid_store.reload({params : {sql : province_city}});			
				}else{
					Create_GridPanel_Remote(year_id,province_city);
				}
			}
		}
	}
	...
	function Create_GridPanel_Remote(_year,_sql) {
		if (Ext.getCmp("mohonlyread_grid_store") != undefined) {
		    Ext.getCmp("mohonlyread_grid_store").remove();
		}
		if (Ext.getCmp("mohonlyread_grid") != undefined) {
		...
		var mohonlyread_grid_store = new Ext.data.Store({//配置分组数据集
			id : 'mohonlyread_grid_store',
			autoLoad: {params:{yearid : _year,sql:_sql}},
			reader: new Ext.data.JsonReader(),
			proxy : new Ext.data.HttpProxy({
				url  : 'GetAllDiseaseMOHGridstorePage'
			})
		});
		var mohonlyread_grid = new Ext.grid.GridPanel({
			title : '卫计委综合医院名称',
		    id: "mohonlyread_grid",
		    autoScroll: true,
		    border: true,
			...
			containerScroll:true,
			trackMouseOver:true,
		    cm: new Ext.grid.ColumnModel(fun_grid_colModel_config("MOHDisease",grid_sm)),
		    store: mohonlyread_grid_store
		});
		Ext.getCmp('dynamic_panel').add(mohonlyread_grid);
		Ext.getCmp('dynamic_panel').doLayout(); 
		
		var hoprightMenu = new Ext.menu.Menu( {  
	    id : 'hoprightMenu',  
	    items : [
	    	{  
		        text:'整行加载复制内容',  
		        ...
		        handler:fun_hop_row_copy_nav  
		    },
	    	{  	        
		        text:'整行加载复制(包括表头)内容',  
		        iconCls:'copy_header',
		        handler:fun_hop_row_copy_header_nav
		    },
		    '-',
		    {text:"导出",iconCls:'export_small',handler : fun_moh_export}
		    ]  
		});
		mohonlyread_grid.addListener('rowcontextmenu', hop_rightClickFn);
		function hop_rightClickFn(grid, rowIndex, e) {  
		    e.preventDefault();  
		    hoprightMenu.showAt(e.getXY());    
		    //grid.getSelectionModel().selectRow(rowIndex);  
		}
		
		function fun_moh_export(){
			//export rights
			Ext.Ajax.request({      
				 url: 'GetexportRows',
		         params: {rights:rightspk},      
		         callback : function(options, success, response) {   
		            if(success){ 
		            	var erows = Ext.getCmp('mohonlyread_grid').store.getCount();
		            	var data = trim(response.responseText);	
		            	exportRowNum = parseInt(trim(data.split("@")[0])); //当前页可导出量
		            	var exportedRows = trim(data.split("@")[1]);  //当月已经导出行数
		            	var exportedObs =  trim(data.split("@")[2]);  //当月已经导出次数	
		            	...
		            	if(exportedObs == ''){
							if(erows > exportRowNum){
							    ...
							    function callback(id){
							   		if(id == 'yes'){
									  exported(Ext.getCmp('mohonlyread_grid'));
										//record export rows						
							    		Ext.Ajax.request({      
							    			 url: 'SaveExportActions',
								             params: {rights:rightspk,exportbutton:3,exportedrows:erows},      
								             callback : function(options, success, response) {}
								        });							  
							   		}
							    }
		...
		function fun_hop_row_copy_nav(){
			ctype = 3;
			fun_row_copy(mohonlyread_grid,"copybutton");
		}
		
		function fun_hop_row_copy_header_nav(){
			ctype = 3;
			fun_row_copy_header(mohonlyread_grid,"copybutton");
			}			
		}
	   ...
	   /***************************
	     * ## 默认展开第一层树
	   ****************************/
		Ext.getCmp("West_DiseaseNativate_Tree").getRootNode().expand(false, true, function() {  
		    var childNodes = West_DiseaseNativate_Tree.getRootNode().childNodes;  
			for(var i=0;i<childNodes.length;i++){
				var path = childNodes[i].getPath('id');
				West_DiseaseNativate_Tree.expandPath(path,"id");
			}
		});  
	...
	function barchart_summit(){
		if(activeaccording_id == 0.0 ){    		
			...
			if(trim(charttypeid).length == 0 || trim(haxisvalue).length == 0 || trim(vaxisvalue).length == 0){
				...
				var sql = grid_sql + '@' + quick_filter_sql + '@' + filter_module_sql + '@' + sort_module_sql + '@' + newcolumn_module_sql + '@' + tree_sql;
	    		Ext.Ajax.request({      
		             //url: '../ActionController/ChartAction/Disease/single_barchart_for_disease.jsp',
	    			 url : 'GetDiseaseSingleBarChart',
				...
				//var data = '([{"label": "中国","value": "10000"},{"label": "Q2","value": "11500"},{"label": "Q3","value": "12500"},{"label": "Q4","value": "15000"}])';
				//renderCharts("column2d",1,'中国','中国','中国','中国',eval(data));									
			}
		}
		else if(activeaccording_id == 0.1){    		
			...
			if(trim(charttypeid).length == 0 || trim(haxisvalue).length == 0 || trim(vaxisvalue).length == 0){
				...
				var sql = grid_sql + '@' + quick_filter_sql + '@' + filter_module_sql + '@' + sort_module_sql + '@' + newcolumn_module_sql + '@' + tree_sql;
	    		Ext.Ajax.request({      
		             //url: '../ActionController/ChartAction/Disease/multi_barchart_for_disease.jsp', 
		             url : 'GetDiseaseMultiBarChart',
		             params: {sql:sql,haxis:haxisvalue,vaxis:vaxisvalue,dbname : database,rights:rightspk},      
		             callback : function(options, success, response) {   
		                if(success){ 
		                	var data = trim(response.responseText);
	...
	function scatterchart_summit(){    	
		if(activeaccording_id == 5.0 ){    		
			...
			if(trim(charttypeid).length == 0 || trim(haxisvalue).length == 0 || trim(vaxisvalue).length == 0){
				...
				var sql = grid_sql + '@' + quick_filter_sql + '@' + filter_module_sql + '@' + sort_module_sql + '@' + newcolumn_module_sql + '@' + tree_sql;
	    		Ext.Ajax.request({      
		             //url: '../ActionController/ChartAction/Disease/scatter_chart_for_disease.jsp',
	    			 url : 'GetDiseaseScatterChart',
	...
	function bubblechart_summit(){    	
		if(activeaccording_id == 7.0 ){    		
			...
			if(trim(charttypeid).length == 0 || trim(haxisvalue).length == 0 || trim(vaxisvalue).length == 0 || trim(zaxisvalue).length == 0){
				...
				var sql = grid_sql + '@' + quick_filter_sql + '@' + filter_module_sql + '@' + sort_module_sql + '@' + newcolumn_module_sql + '@' + tree_sql;
	    		Ext.Ajax.request({      
		             //url: '../ActionController/ChartAction/Disease/bubble_chart_for_disease.jsp',
	    			 url : 'GetDiseaseBubbleChart',
	...
	function combinedchart_summit(){
		if(activeaccording_id == 9.0 ){    		
			...
			if(trim(charttypeid).length == 0 || trim(haxisvalue).length == 0 || trim(vaxisvalue).length == 0){
				...
				var sql = grid_sql + '@' + quick_filter_sql + '@' + filter_module_sql + '@' + sort_module_sql + '@' + newcolumn_module_sql + '@' + tree_sql;
	    		Ext.Ajax.request({      
		             //url: '../ActionController/ChartAction/Disease/single_combinedchart_for_disease.jsp',
	    			 url : 'GetDiseaseSingleCombinedChart',
			...
			if(trim(charttypeid).length == 0 || trim(haxisvalue).length == 0 || trim(vaxisvalue).length == 0){
				...
				var sql = grid_sql + '@' + quick_filter_sql + '@' + filter_module_sql + '@' + sort_module_sql + '@' + newcolumn_module_sql + '@' + tree_sql;
	    		Ext.Ajax.request({      
		             //url: '../ActionController/ChartAction/Disease/multi_combinedchart_for_disease.jsp', 
		             url : 'GetDiseaseMultiCombinedChart',
	   ...
	   /***************************
	     ...
	     *   	$$render_multipiechart(_grid,_FieldStr_Point): 多维度饼图生成
	...
	function render_multicombinedchart(_grid,_FieldStr_Point){		
	   ...
	   /***************************
	     * ## 疾病治愈率地图
	   ***************************/
	function Create_morbidityMap(){
		if (Ext.getCmp("morbidity_winpanel") != undefined) {
		    Ext.getCmp("morbidity_winpanel").destroy();
		}		
		var sql = tree_sql;//grid_sql + '@' + quick_filter_sql + '@' + filter_module_sql + '@' + sort_module_sql + '@' + newcolumn_module_sql + '@' + tree_sql;
		sql = replaceAll(sql," ","$");
		var caption = "全国重点城市疾病治愈率(每10万人)";				
		var morbidity_winpanel = new Ext.Window({ 
		        title : '治愈率地图(' + tree_node+")", 
		        id:'morbidity_winpanel',
		        width : 850,  
		        height: 500, 
		        resizable:false,
		        closeAction: 'hide',
				...
				fill : true,
	      		layout : 'fit',
	      		items:[{html:'<iframe src=Disease/ifrom_map/map.jsp?caption='+encodeURI(encodeURI(caption))+'&sql='+encodeURI(encodeURI(sql))/*+'&dbname='+database*/+' width="100%" height="100%" frameborder="no" scrolling="no"></iframe>'}]
		});	
	}	
	
	function fun_morbidity(){
		Create_morbidityMap();
		//Ext.getCmp('morbidity_winpanel').add({html:'<iframe src=Disease/ifrom_map/map.jsp?caption='+encodeURI(encodeURI(caption))+'&sql='+encodeURI(encodeURI(sql))/*+'&dbname='+database*/+' width="100%" height="100%" frameborder="no" scrolling="no"></iframe>'});
		//Ext.getCmp('morbidity_winpanel').doLayout(); 		
		Ext.getCmp('morbidity_winpanel').show();
		}	
		...
		if(crows == 0){
		    ...
		    function callback(id){
		   		if(id == 'ok'){
					Ext.Ajax.request({      
						 url: 'SaveCopyActions',
			             params: {rights:rightspk,copybutton:ctype,copyedrows:crows},      
			             callback : function(options, success, response) {}
			        });
			        crows = 0;
		   		}
