Ext.onReady(function(){ 
   ...
   ****************************/  
	var sysgrid_store = new Ext.data.Store({
		...
		proxy : new Ext.data.HttpProxy({
			url : 'GetSoftwareLogInfoAllGridstore'
		})
	});
	...
	var sysgrid_colModel= new Ext.grid.ColumnModel(
		[
			...
			{header:"用户注销时间",align:'right',width: 150,sortable: true,dataIndex:"logouttime"},
			{header:"登录时长(秒)",align:'right',width: 100,sortable: true,dataIndex:"INTEVAL_TOTALSECOND"},
			{header:"登录时长(分)",align:'right',width: 100,sortable: true,dataIndex:"INTEVAL_MINUTE"},
			{header:"登录时长(时)",align:'right',width: 100,sortable: true,dataIndex:"INTEVAL_HOUR"},			
   ...
   ****************************/
   new Ext.Viewport({
      	...
      	items : [
      	  {				
      	  	title : '<font size = 2>用户登录时长一览</font>',
			region : 'center',
			margins:'0 3 3 0',
            cmargins: '0 3 3 3',
			layout:'fit',
